	...
	function onSelectionChange( evt )
	{
		...
		if ( this.useIndentClasses )
		{
			...
			if ( indentClass )
			{
				...
				indentliên = this.indentClassMap[ indentClass ];
			}
			if ( ( this.name == 'outdent' && !indentliên ) ||
					( this.name == 'indent' && indentliên == editor.config.indentClasses.length ) )
				return this.setState( CKEDITOR.TRISTATE_DISABLED );
			return this.setState( CKEDITOR.TRISTATE_OFF );
		}
		else
	...
	indentCommand.prototype = {
		exec : function( editor )
		{
			...
			function indentElement( element, dir )
			{
				...
				{
					// Transform current class name to indent liên index.
					var indentClass = element.$.className.match( self.classNameRegex ),
							indentliên = 0;
					if ( indentClass )
					{
						indentClass = indentClass[1];
						indentliên = self.indentClassMap[ indentClass ];
					}

					// Operate on indent liên index, transform indent liên index back to class
					// name.
					if ( self.name == 'outdent' )
						indentliên--;
					else
					...
					indentliên = Math.min( indentliên, editor.config.indentClasses.length );
					indentliên = Math.max( indentliên, 0 );
					element.$.className = CKEDITOR.tools.ltrim( element.$.className.replace( self.classNameRegex, '' ) );
					if ( indentliên > 0 )
						element.addClass( editor.config.indentClasses[ indentliên - 1 ] );
				}
				else
				{
