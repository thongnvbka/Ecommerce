function ImportWarehouseAddModel(callback, packageDetailModal, orderDetailModal, walletDetailModal, orderServiceOtherModal) {
    ...
    self.showAddPackageCallback = function (data) {
        ...
        self.addPackageForm().show(data, function() {
            self.transportCodes.remove(data);
        });
    }
    ...
    self.showAddPackageLoseCallback = function (data) {
        ...
        self.addPackageLoseForm().show(data, function() {
            self.transportCodes.remove(data);
        });
    }
    ...
    self.save = function() {
        ...
        _.each(data.OrderServiceOthers, function (s) {
            ...
            if (s.orderCode.length === 0 || s.value.length === 0) {
                hasError = true;
                toastr.warning(" Orders code  and the value of money in the charges incurred may not be blank");
                return false;
            }
                    ...
                    if (isPaste) {
                        if (result.length > 0) {
                            ...
                            if (_.trim($('#suggetion').val("")).length > 0) {
                                self.transportCodes.push(_.trim(request.term));
                            }
                        }
    ...
    self.processItemSelected = function(item) {
        ...
        if (self.id() > 0) {
            ...
            package.importWarehouseId = self.id();
            package.importWarehouseCode = self.code();
            package["__RequestVerificationToken"] = self.token;

            self.isLoading(true);
            $.post("/importwarehouse/addpackage", package, function (rs) {
                self.isLoading(false);
                if (rs.status < 0) {
                    toastr.error(rs.text);
                    return;
                }
                self.getDetail(package.packageId);
            });
            return;
        }
