...
$(function () {
    ...
    if (currentDomain === '1688') {
        ...
        if (typeof tbeInventory == 'number') {
            tbePrice += '<dl><dd style="width:100%"><span class="text-danger">Shop hiện còn <b>' + tbeInventory + '</b> sản phẩm</span></dd></dl>';
        }
        if (typeof objPrice == 'number') {
            ...
            if (objPrice.length > 0) {
                if (parseInt(objPrice[0].begin) > 1) {
                    ...
                    tbePrice += '<dl><dd style="width:100%"><span class="text-danger">Shop yêu cầu mua tối thiểu ' + objPrice[0].begin + ' sản phẩm</span></dd></dl>';
                }
                for (var i = 0; i <= objPrice.length - 1; i++) {
                    if (objPrice[i].end.length > 0) {
                        tbePrice += '<dl><dd>Mua: ' + objPrice[i].begin + ' - ' + objPrice[i].end + ' sản phẩm</dd><dd>Giá: <span class="tbe-color-price">¥' + objPrice[i].price + '</span></dd></dl>';
                    } else {
                        tbePrice += '<dl><dd>Mua: &gt;' + objPrice[i].begin + ' sản phẩm</dd><dd>Giá: <span class="tbe-color-price">¥' + objPrice[i].price + '</span></dd></dl>';
                    }
                }
            }
        ...
        if (typeof tbeInventory == 'number') {
            if (currentDomain === 'TMALL') {
                ...
                tbePrice += '<dl><dd style="width:100%"><span class="text-danger">Shop giới hạn mua tối đa <b class="tbe-color-price">' + tbeInventory + '</b> sản phẩm</span></dd></dl>';
            }
        }
    ...
    $('#J_PromoPrice, #J_priceStd, #J_PromoWrap').on('DOMNodeInserted', function (ev) {
        ...
        if (newPrice > 0) {
            ...
            if ($('#tbe-warning-bar:hidden')) {
                showTbeMeasage('Thông báo', '<span class="text-white"><strong>Chú ý:</strong> Sản phẩm vừa được cập nhật giá theo thuộc tính đã chọn, vui lòng kiểm tra lại thông tin!</span>');
    ...
    //favorites_20160307
    var btnNotes = '<div id="tbe-notes"><dl><dt>Ghi chú</dt><dd><textarea id="tbex-notes" name="tbex-notes" placeholder="Ghi chú cho sản phẩm này vào likeorder.com" rows="5" cols="50" maxlength="2000"></textarea></dd></dl></div>';
    ...
    var btnShowCart = makeButton({
        ...
        type: 'button'
    }, '<span class="bg-img-cart"></span> Giỏ hàng', function (ev) {
        ev.preventDefault();
        var tab = window.open(GDTQ + '/Product/Cart', '_blank');
        tab.focus();
    });
    ...
    var btnSubmit = makeButton({
        ...
        type: 'button'
    }, '<span class="bg-img-add"></span> Thêm vào giỏ hàng', order);
    ...
    lblWarning
       ...
       .html('<span class="text-white"><strong>Chú ý:</strong> Sản phẩm vừa được cập nhật giá theo thuộc tính đã chọn, vui lòng kiểm tra lại thông tin!</span>')
       .hide()
       .on('click', function () {
           $(this).hide();
       });
    var khuyencao2 = '<a title="likeorder" target="_blank" href="' + GDTQ + '"><img id="tbe-logo" src="' + GDTQ + '/images/logo_likeorder.png"/></a><span class="text-danger text-white"><strong>Cảnh báo: </strong>Bạn không dùng google translate khi thêm sản phẩm!</span>';
...
function checkFor1688Com() {
    ...
    if (totalAmount === 0) {
        showTbeMeasage('Thông báo', 'Bạn chưa chọn số lượng sản phẩm cần muaอ.');
        scrollToAlert();
        return false;
    } else {
        var hasError = false;
        ...
        if (tableSku.length > 0) {
            for (var i = 0; i < tableSku.length; i++) {
                ...
                if (qty > maxValue) {
                    showTbeMeasage('Thông báo', 'Shop chỉ còn "' + maxValue + '" ' + objTitle + ' "' + tmp.skuName + '"');
                    hasError = true;
                    return false;
                }

                if (qty < tmp.min) {
                    showTbeMeasage('Thông báo', 'Shop yêu cầu mua ít nhất "' + tmp.min + '" ' + objTitle + ' "' + tmp.skuName + '"');
                    hasError = true;
                    return false;
                }
            }
